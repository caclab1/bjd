...
contract Bugatti is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = 198262 * 10**9; 
    ...
    // IERC20 public usdt = IERC20(0xca74b5e5F5fcC29C23cA2f10A4059D3FA23FFd37);
    IWrap public wrap;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFomo, uint256 tDev, uint256 tLiquidity, uint256 tFee) {
        ...
        }tFomo = tAmount.mul(20).div(1000);
        tDev = tAmount.mul(16).div(1000);
        tLiquidity = tAmount.mul(20).div(1000);
        tFee = tAmount.mul(24).div(1000);
    ...
    function _takeTax(uint256 tFomo, uint256 tDev, uint256 tLiquidity) private {
        ...
        _rOwned[BurnReceiver] = _rOwned[BurnReceiver].add(rFomo);
        if (_isExcluded[BurnReceiver]) {
            _tOwned[BurnReceiver] = _tOwned[BurnReceiver].add(tFomo);
        }
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && to != owner() && to != BurnReceiver)
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (tFee > 0) {
            emit Transfer(sender, BurnReceiver, tFomo);
